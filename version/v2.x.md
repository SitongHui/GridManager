...
- setTableData 方法需要监听数据是否被修改， 并通过监听数据变化来修改DOM: 数组字段为empty时，循环不会被触发
- renderTableBody 改造为 createElement 模式, 并使用document.createDocumentFragment() 提升性能
- renderTableBody 中添加当前数据是否已存在的验证，如果不存在则替换DOM
- updateTrDOM 可以利用起来，用于处理DOM节点已存在，但数据已改变的情况
- 替换 setTableData 的使用场景


文档API增加分组
- 考虑把参数、事件、方法进行整合，以功能进行重新分组
- 分组项
    - 基础实现
    - 
